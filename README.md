...
| POST | `/v1/trading-book-management-plan/initiate` | Initiate |
| GET | `/v1/trading-book-management-plan` | Retrieve (list) |
| GET | `/v1/trading-book-management-plan/{crId}/retrieve` | Retrieve |
| PUT | `/v1/trading-book-management-plan/{crId}/update` | Update |
| PUT | `/v1/trading-book-management-plan/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/trading-book-management-plan/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-trading-book-management:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-trading-book-management ./helm -n bian-operations
```

Exposed through the platform Gateway at path prefix `/sd-trading-book-management` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
